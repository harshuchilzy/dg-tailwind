...
//bussiness dates
include get_stylesheet_directory() . '/includes/holiday-calendar.php';

add_action('rest_api_init', function () {
    register_rest_route('dev/dg/v2', 'get-holidays', array(
        'methods'  => 'GET',
...
function get_dg_holidays()
{
    $objectify = array();
    foreach (json_decode(get_option('dg_holidays')) as $holiday) {
        $date = new stdClass();
        $date->start = $holiday;
        $date->overlap = false;
...
function search_item_id()
{
    ...
    $search_product_id = $_POST['product_id'];
    $product = wc_get_product($search_product_id);
    if ($product->get_type() == 'variable') {
        $variations = $product->get_available_variations();
        $variations_id = wp_list_pluck($variations, 'variation_id');
    }
    echo json_encode($variations_id);
    wp_die();
}
add_action("wp_ajax_search_item_id", "search_item_id");
